...
local Node = {
	...
	fail = nil,
	char = "",
	tag = 0, --tag=0表示非可接收态,1表示可接收态
	No = 0,
}

function Node:new(node)
	local new_node = node or {}
	self.__index = self
...
NodeToQueue = function( _M, root, queue )
	for k,v in pairs(root.nexts) do
		queue:Push(v)
	end
end

BuildTree = function(_M, all_patterns)
	local root = Node:new()
	local tmp1, tmp2
	for k,pattern in ipairs(all_patterns) do
		tmp1 = root
		for j = 1, #pattern do
			local char = string.byte(pattern, j, j)
			if tmp1.nexts[char] == nil then
				tmp2 = Node:new()
				tmp2.nexts = {}
				tmp2.tag = 0
				tmp2.No = 0
				tmp2.char = char
				tmp2.parent = tmp1
				tmp1.nexts[char] = tmp2
				tmp1 = tmp2
			else
				tmp1 = tmp1.nexts[char]
			end
		end
		tmp1.tag = 1
		tmp1.No = k
...
BuildFailPath = function (_M, root)
	local queue = Queue:new()
	root.fail = root

	for k,v in pairs(root.nexts) do
		NodeToQueue(_M, v, queue)
		v.fail = root
	end
	...
	while not queue:Empty() do
		tmp = queue:Pop()
		NodeToQueue(_M, tmp, queue)

		input_char = tmp.char
		parent = tmp.parent
		while true do
			if parent.fail.nexts[input_char] ~= nil then
				tmp.fail = parent.fail.nexts[input_char]
				break
			else
				if parent.fail == root then
					tmp.fail = root
					break
				else
...
SearchAC = function ( _M, root, str)
	...
	local i = 0
	while i <= len do
		local char = string.byte(str, i, i)
		if tmp.nexts[char] then
			tmp = tmp.nexts[char]
			if tmp.tag == 1 then
				-- return true, tmp.No
				table.insert(index_list, tmp.No)
				-- table.insert(list, i)
			end
			i = i + 1
		else
...
local root = BuildTree(_M, all_patterns)
BuildFailPath(_M, root)

HasSensitiveWords = function( _M, str)
	local index_list = SearchAC(_M, root, str)
	-- Destroy(_M, root)
	if index_list and type(index_list) == "table" and #index_list > 0 then
		return true
	else
		return false
	end
...
ReplaceSensitiveWords = function(_M, str)
	local index_list = SearchAC(_M, root, str)
	local result = str
	for k,v in ipairs(index_list) do
		local len = #all_patterns[v]
		result = string.gsub(result, all_patterns[v], string.rep("*", math.floor(len/2)))
	end
	return result
end
...
Main = function (_M)
	...
	for k,v in ipairs(index_list) do
		local len = #all_patterns[v]
		x = string.gsub(x, all_patterns[v], string.rep("*", len))
	end
end
