...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		{	
			...
			foreach(string libRef in mod.libs ) {
                string[] filename = libRef.Split(':');
                bool isWeak = (filename.Length > 1) ? true : false;
                string completeLibPath = System.IO.Path.Combine( "usr/lib", filename[0]);
				Debug.Log ("Adding library " + completeLibPath);
				this.AddFile( completeLibPath, frameworkGroup, "SDKROOT", true, isWeak );
			}
